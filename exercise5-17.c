...
int directory_order = 0; /* 1 if makes comparisons only on letters, numbers and blanks */
int column = 0; /* n column to be considere at sorting the lines */

int strcmp_fd(const char *s1, const char *s2) 
{  
  extern int fold;
  extern int directory_order;
  extern int column;

  for (int column_s1 = 0; column_s1 < column; column_s1++)
    while (*s1 && *s1 != ' ' && *s1 != '\t' && *s1 != '\n' && *s1 != '\0')
      *s1++;
    if (*s1 && *s1 != '\0')
      *s1++;

  for (int column_s2 = 0; column_s2 < column; column_s2++)
    while (*s2 && *s2 != ' ' && *s2 != '\t' && *s2 != '\n' && *s2 != '\0')
      *s2++;
    if (*s2 && *s2 != '\0')
      *s2++;
...
int main (int argc, char *argv[])
{
  ...
  if ((nlines = readlines(lineptr, MAXLINES)) >= 0) {
    qsort1((void **) lineptr, 0, nlines-1, (int (*)(void *, void *))(numeric ? numcmp : strcmp_fd));
